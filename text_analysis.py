...
with open(synonyms_path, 'rb') as file:
  dict_synonyms = pickle.load(file)
with open(base_lemmas_path, 'rb') as file:
  dict_base_lemmas = pickle.load(file)
with open(phonemes_path, 'rb') as file:
  dict_phonemes = pickle.load(file)
with open(stop_words_path,'rb') as file:
  dict_stop_words = pickle.load(file)
  # print(dict_stop_words)
# type(nlp.Defaults.stop_words) = <class 'set'> (sure)
nlp.Defaults.stop_words = dict_stop_words
with open(frequent_words_pickle_path,'rb') as file:
  dict_word_freq = pickle.load(file)
with open(filler_words_path, 'rb') as file:
  dict_fillers = pickle.load(file)

dict_weak_verbs = {'is', 'are', 'am','been', 'be','were', 'was'}
dict_entity_substitutions = {'this', 'that', 'you', 'he', 'she', 'it'}
# with open(os.path.join(os.environ.get('NLP_DATA'), 'weak-verbs')) as file:
#   dict_weak_verbs = file.read().splitlines()
#     # weak-verbs is a collection: overused vague verbs
# with open(os.path.join(os.environ.get('NLP_DATA'), 'entity-substitutions')) as file:
#   dict_entity_substitutions = file.read().splitlines()
#   # dict_entity_substitutions is a collection: pronouns and vague determiners


# dict_synonyms and dict_stop_words have '-'
...
def analyze_text(html):
  ...
  for idx, token in enumerate(doc):
    # idx, token  = (0, '...')(1, '...')(2, '...')... respectively
# type(doc) = spacy.tokens.doc.Doc
# type(data['...'][idx]) = word = child of type(list<token.lower>) (words)
    if data['words'][idx]:
  ...
  for idx_word, word in enumerate(words):
    ...
    # type(dict_synonyms) = dict[str, dict]
  ...
  if word_freq_list:
    ...
    # => type(metrics['word_freq']) = list
  else:
    metrics['word_freq'] = []
  bigram_freq_list = []
  for mask, freq in bigram_freq.items():
    w1, w2 = mask.split('|')
  ...
  if bigram_freq_list:
    ...
    metrics['bigram_freq'] = []
  trigram_freq_list = []
  for mask, freq in trigram_freq.items():
    w1, w2, w3 = mask.split('|')
    if (freq > 1):
